...
import { Alert, StyleSheet, Text, View, FlatList, ImageBackground, TouchableOpacity, ActivityIndicator } from 'react-native';
...
const formatFechaES = (fecha = new Date()) => {
  const meses = [
...
export default function SelectWorkout() {
  ...
  const userId = contexto?.userContext?.id;
  const API_URL = contexto?.API_URL;
  const fechaDeHoy = useMemo(() => formatFechaES(new Date()), []);

  // Fetch user profile data
  const handleSession = async () => {
    if (!contexto?.API_URL) return;

    try {
      const response = await fetch(`${contexto.API_URL}/perfil/perfil`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mail: contexto.userContext.mail,
          password: contexto.userContext.password
        })
      });
  ...
  const fetchUserRoutines = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/crearEj/routines/${userId}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      });
    ...
    <View style={[styles.container, { backgroundColor: theme.overlay, width: "100%" }]}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.modeButtons}>
          <TouchableOpacity onPress={() => setMode('image')}>
            <Ionicons name="image" size={26} color={mode === 'image' ? theme.accent : theme.textMuted} style={styles.modeIcon} />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setMode('dark')}>
            <Ionicons name="moon" size={26} color={mode === 'dark' ? theme.accent : theme.textMuted} style={styles.modeIcon} />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setMode('light')}>
            <Ionicons name="sunny" size={26} color={mode === 'light' ? theme.accent : theme.textMuted} style={styles.modeIcon} />
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={[styles.content, { width: "100%" }]}>
        <Boton
          ...
          viewStyle={[styles.mainCard, { backgroundColor: theme.accent, shadowColor: theme.text }]}
        >
        ...
        <View style={styles.row}>
          <Boton
            ...
            viewStyle={[styles.smallCard, { backgroundColor: theme.success, shadowColor: theme.text }]}
          >
          ...
          <Boton
            ...
            viewStyle={[styles.smallCard, { backgroundColor: theme.warning, shadowColor: theme.text }]}
          >
        ...
        <View style={styles.sectionHeader}>
          ...
          {loadingRoutines && <ActivityIndicator color={theme.accent} size="small" />}
        </View>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', alignItems: 'center', marginVertical: 10 },
  ...
  workoutTitle: { fontSize: 16, fontWeight: '600', marginTop: 8, textAlign: 'center' },
});